...
static const int nmax_proj = 20; // original 5
static const int nmax_2d   = 2*nmax_proj; // original
...
bool Peaks2D::HandleExec()
{
    ...
    // ABOVE: Hack by JEM to run NextDetector()
    return true;
}
...
TH2* Peaks_EDE::NextDetector()
{
    // Hack by Jørgen EM: Copied from NextMatrix, but skips straight to the next 
    // detector piece (front and/or back) even if nPeaks has not yet been reached.
...
TH2* PeaksAny::NextDetector()
{
    // JEM hack: Copy of PeaksAny::NextMatrix()
    iPeak += 1;
    return matrix;
}
...
TH2* Peaks_NAI::NextDetector()
{
    // JEM hack: This function is copy-pasted from PeaksNAI::NextMatrix(), just to make things run. Should be edited if NextDetector functionality is needed for the NAI part.
    if( debug )
        cout << "Peaks_NAI::NextMatrix start: i=" << iPeak << " d=" << iDet << endl;
...
void Peaks_NAI::ShowPeak(double px2d, double py2d, double pxp, double pyp)
{
    ...
    if( iPeak == 0 ) {
        ...
        if( outfilename ) {
            ofstream fout(outfilename, ios::app|ios::out);
            fout << iDet << '\t' << pxp << '\t' << px4 << '\t' << pyp << '\t' << py4 << endl;
