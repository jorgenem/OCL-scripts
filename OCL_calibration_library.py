...
def line_up_2points(peaks_file1, peaks_file2, peak1_coordinates, peak2_coordinates, g0_front = 2.5, g0_back = 5):
...
def calibrate_2points_from_list_summed_backs(peaks1, peaks2, target1, target2, reaction1, reaction2, excitation_number1, excitation_number2, g0_front = 2.5, g0_back = 5):
	# This function takes two peaks and two matching Qkinz calculated levels, and outputs
	# gains and shifts. 
...
def rebin_data(data, factor):
	...
	if factor == 1:
		...
		Nbins = len(data[0,:])
		data_new = np.zeros((4, Nbins/factor))
...
def line_up_ede(spectra, i_current,  i_reference=0, Nbins=[1000], binshift=[0], rebin=1, error_option=1, shift_width=50, show_plots=True, normalize_spectra=True):
	...
	if show_plots:
		...
		plt.figure(1)
		plt.plot(reference[0,reference_indices.astype(int)], reference[1,reference_indices.astype(int)])
		plt.plot(current[0,reference_indices.astype(int)], current[1,current_indices.astype(int)])
		plt.plot(current[0,reference_indices.astype(int)], current[1,reference_indices.astype(int)])
		plt.title('Spectra')
		plt.legend(['Reference', 'Current, calibrated', 'Current, uncalibrated'], fontsize=12, loc='upper left')
		if error_option == 3:
			plt.yscale('log')
...
def line_up_nai(spectra, i_current,  i_reference=0, Nbins=[1000], binshift=[0], rebin=1, error_option=1, shift_width=50, show_plots=True, show_error_plots=False, normalize_spectra=True):
	...
	if show_error_plots:
		plt.figure(2)
		plt.step(reference[0,reference_indices.astype(int)], np.power(reference[1,reference_indices.astype(int)]-current[1,reference_indices.astype(int)],2), color='darkseagreen')
		plt.step(reference[0,reference_indices.astype(int)], np.power(reference[1,reference_indices.astype(int)]-current[1,current_indices.astype(int)],2), color='crimson')
		plt.title('Squared error')
		plt.legend(['reference minus uncalibrated', 'reference minus calibrated'], fontsize=12, loc='best')
...
def calibrate_ede_strip(spectra, strip, Nbins=[1000], rebin=1, binshift=[0], error_option=1, shift_width=50, show_plots=True, normalize_spectra=True):
	...
	if show_plots:
		...
		plt.xlim([spectra[strip][1,0], spectra[strip][1,max((binshift[0]+Nbins[0])*1.2, N*0.95)]])
		if error_option == 3:
			plt.yscale('log')
