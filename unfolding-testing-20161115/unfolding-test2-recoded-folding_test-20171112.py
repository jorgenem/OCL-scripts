...
Ex_mesh, Eg_mesh = np.meshgrid(Ex_range, Eg_range, indexing='ij')
print Eg_mesh.shape, Ex_mesh.shape

# Make a simple gaussian 2D peak:
# raw = 500*norm.pdf(Eg_mesh, loc=500, scale=50, axis=1) * norm.pdf(Ex_mesh, loc=100, scale=50, axis=0)
pos = np.empty(Eg_mesh.shape + (2,))
pos[:, :, 0] = Eg_mesh; pos[:, :, 1] = Ex_mesh
sigmaEx = 20
sigmaEg = 50
Eg_center = 2000
Ex_center = 150
raw = 1e5*(1/(2*np.pi*sigmaEx*sigmaEg) * np.exp(-((Ex_mesh-Ex_center)**2/(2*sigmaEx**2)
     + (Eg_mesh-Eg_center)**2/(2*sigmaEg**2))))
raw = Ex_mesh
print Ex_range
...
# Fold synthetic data with R for testing:
folded = np.dot(R.T, rawmat)

plt.figure(20)
plt.subplot(1,2,1)
plt.pcolormesh(Eg_range[IEg_low:IEg_high], Ex_range[IEx_low:IEx_high], rawmat.T)#, norm=colors.LogNorm(vmin=1e-10, vmax=rawmat.max()))
plt.subplot(1,2,2)
plt.pcolormesh(Eg_range[IEg_low:IEg_high], Ex_range[IEx_low:IEx_high], folded.T)#,  norm=colors.LogNorm(vmin=1e-10, vmax=folded.max()))
